...
north_deque = new_deque(north)
center_deque = new_deque(center)
south_deque = new_deque(south)

print (len(north_deque))
print (len(center_deque))
print (len(south_deque))

def comparison_lists (*args):
    biggest_list = args[0]
    biggest_count = len(args[0])
   
    for i in args:
        if len(i) > biggest_count:
            biggest_list = i
            biggest_count = len(i)
    return biggest_count

print (comparison_lists(north_deque,center_deque,south_deque))

from collections import Counter

north_counter = Counter(north_deque)
center_counter = Counter(center_deque)
south_counter = Counter(south_deque)
...
def large_number_product (*args):
    large_number_deque = deque()
    a = args[0]
    b = args[1]  
    
    for i in a:
        if a[i]>b[i]:
            large_number_deque.append({i:a[i]}) 
    return large_number_deque

            
print (large_number_product(north_counter, center_counter))

def most_popular_product (*args):
    common_product=[]
    for i in args:
        a=i.most_common(1)
        common_product.append(a[0])
    
    sum_product = 0
    most_common_product = []
    for i in common_product:
        for j in args:
            sum_product += j[i[0]]
        if i[1]>sum_product-i[1]:
            most_common_product.append(i)
    return most_common_product
print(most_popular_product(north_counter, center_counter, south_counter))

general_sales = north_counter+center_counter+south_counter
most_common_products = general_sales.most_common(2)
